...
public class CartServiceBox implements Service {
	private static final Logger LOGGER = LoggerFactory.getLogger(CartServiceBox.class);
	protected ObjectAdapter _adapter;
	protected Identity id;

	@Override
	public void start(String name, Communicator communicator, String[] arg2) {
		LOGGER.info("start  servant :CartServiceI");
		// IceBox
		// 创建objectAdapter，这里和service同名
		_adapter = communicator.createObjectAdapter(name);
		// 创建servant
		com.zeroc.Ice.Object object = new CartServiceI();
		id = Util.stringToIdentity(name);
		// _adapter.add(object, communicator.stringToIdentity(name));
		_adapter.add(object, id);
		// 激活
		_adapter.activate();
		LOGGER.info("start  servant success :CartServiceI");
